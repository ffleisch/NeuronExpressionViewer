...
public class WhereMousePoint : MonoBehaviour
{
    ...
    private void setConnectionsShown()
    {
        ...
        area_index = (int)(LoadTextures.ColorToFloat(col) / 50000);//todo make number neurons dynamic
        Debug.Log(index + " " + area_index);
    ...
    public void setShaderParams()
    {
        foreach (Transform t in transform)
        {
            ...
            if (renderer)
            {
                renderer.sharedMaterial.SetFloat("_pixelWidth", lineWidth);
                renderer.sharedMaterial.SetInteger("_n_index", index);
                renderer.sharedMaterial.SetInteger("_n_area", area_index);
            }
        }
    }
